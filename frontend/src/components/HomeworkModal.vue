<template>
  <div class="container">
    <font-awesome-icon
      v-if="homework.userno == user"
      class="fa-2x mt-1 mr-1 float-left"
      icon="edit"
    />
    <a
      v-if="homework.userno == user"
      ...
      @click="showModal"
      ><strong>과제 추가하기</strong></a
    >
    <!--수정-->
    <b-modal ref="my-modal" ok-only title="Create Homework" hide-footer>
      <form>
  ...
  methods: {
    ...
    createHomework: function () {
      ...
      if (
        ...
        axios({
          method: "post",
          url: `${process.env.VUE_APP_SERVER_URL}homework/create`,
          data: this.homework,
