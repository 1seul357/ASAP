    ...
    <table class="table table-bordered">
      <tbody>
        ...
        <tr>
          ...
          <td>
            <user-interests
              :interestList="getInterests(member)"
            ></user-interests>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td v-if="studylist.length >= 1">
            {{ studylist[0].studyname
            }}<a
              v-if="isAdmin == 1"
              type="button"
              @click="userKick(studylist[0].studyno, member.userno)"
              >퇴출</a
            >
          </td>
          <!--수정-->
        ...
        <tr v-for="(study, index) in studylist" :key="index.id">
          <td v-if="index != 0 && studylist.length >= 1">
            {{ study.studyname
            }}<a
              v-if="isAdmin == 1"
              type="button"
              @click="userKick(study.studyno, member.userno)"
              >퇴출</a
            >
          </td>
          <!--수정-->
