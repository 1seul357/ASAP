<template>
  <div id="container">
    <div
      class="content"
      :style="{ height: post.studyBoard.content.length + 300 + 'px' }"
    >
      <h5>
        <strong>작성자 : {{ post.nickname }}</strong>
      </h5>
      <div class="overflow" style="width: 100%; white-space: pre-line">
        {{ post.studyBoard.content }}
      </div>
      <hr />
      <h5 class="mb-1">
        <strong
          ><font-awesome-icon icon="fa-solid fa-download" class="fa-xl" />
          첨부파일</strong
        >
      </h5>
      <div v-for="file in post.studyBoard.fileList" :key="file.id">
        <div class="overflow" style="width: 100%; white-space: normal">
          <a type="button" @click="downloadFile(file)"
            ><p class="mb-0">{{ file.ogfilename }}</p></a
          >
          <!--수정-->
        </div>
      </div>
    ...
    <div class="mt-2 mb-3">
      <b-button
        v-if="userno === articleuserno"
        @click="deleteArticle(post.studyBoard.boardno)"
        >삭제</b-button
      >
      <b-button v-if="userno === articleuserno" id="show-btn" @click="showModal"
        >수정</b-button
      >
    </div>
    <b-modal ref="my-modal" ok-only title="Update Ariticle" hide-footer>
      <b-form-group label="Title" label-for="title">
        <b-form-input id="title" type="text" v-model="article.title">
        </b-form-input>
      </b-form-group>
...
export default {
  ...
  methods: {
    ...
    deleteArticle: function (boardno) {
      axios({
        method: "delete",
        url: `${process.env.VUE_APP_SERVER_URL}study_board/delete/${boardno}`,
    ...
    downloadFile: function (file) {
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}study_board/download/${file.fileno}`,
        headers: this.setToken(),
        responseType: "blob",
      })
        .then((res) => {
    ...
    updateArticle: function () {
      ...
      for (var j = 0; j < this.article.content.length; j++) {
        if (this.article.content[j] != " ") {
          flagContent = true;
          break;
        }
      }
      ...
      if (
        ...
        axios({
          method: "put",
          url: `${process.env.VUE_APP_SERVER_URL}study_board/modify`,
          headers: this.setToken(),
          data: formData,
        })
          .then((res) => {
