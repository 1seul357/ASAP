    ...
    <div class="container d-flex justify-content-space-between" id="screen">
      <div class="d-flex align-items-stretch justify-content-between col-2">
        <div class="icon">
          <font-awesome-icon
            type="button"
            class="fa-2x my-2 ml-2"
            @click="sidebarArticleOn"
            icon="edit"
          />
          <b-sidebar
            id="sidebar-right"
            v-model="sidebarArticle"
            title="스터디 게시판"
            right
            shadow
          >
          ...
          <font-awesome-icon
            type="button"
            class="clipboard fa-2x my-2"
            @click="sidebarHomeworkOn"
            icon="clipboard"
          />
          <b-sidebar
            id="sidebar-right-homework"
            v-model="sidebarHomework"
            title="과제 게시판"
            right
            shadow
          >
          ...
          <font-awesome-icon
            type="button"
            class="fa-2x my-2"
            @click="sidebarDetailOn"
            icon="info-circle"
          />
          <b-sidebar
            id="sidebar-left-study"
          ...
          >
            <div>
              <div
                id="information"
                v-for="study in studyInformation"
                :key="study.id"
              >
                <p class="studyInfo">
                  <font-awesome-icon icon="tag" class="fa-1x" /> Name
                </p>
                <p>{{ study.studyname }}</p>
                <p class="studyInfo">
                  <font-awesome-icon icon="user-tag" class="fa-1x" /> Creator
                </p>
                <p>{{ study.maker }}</p>
                <p class="studyInfo">
                  <font-awesome-icon icon="palette" class="fa-1x" /> Category
                </p>
                <p>{{ study.category }}</p>
                <p class="studyInfo">
                  <font-awesome-icon icon="list-ol" class="fa-1x" /> Interests
                </p>
                <p>
                  <b-badge
                    type="info"
                ...
                <p class="studyInfo">
                  <font-awesome-icon icon="newspaper" class="fa-1x" />
                  Description
                </p>
          ...
          <font-awesome-icon
            type="button"
            class="fa-2x my-2"
            @click="sidebarMemberOn"
            icon="user-friends"
          />
          <b-sidebar
            id="sidebar-left-member"
            v-model="sidebarMember"
            title="스터디 회원 목록"
            right
            shadow
          >
            <div
              ...
              v-for="members in studyMemberList"
              :key="members.id"
            >
              <div
              ...
              >
                <div class="d-flex">
                  ...
                  <a
                    ...
                    type="button"
                    class="mt-2 ml-3"
                    @click="showModal(member.studyMember)"
                    >{{ member.nickname }}</a
                  >
                </div>
                <b-button
                  class="mt-1"
                  v-if="
                    userNumber == getLeader() &&
                    member.studyMember.userno != getLeader()
                  "
                  style="
                    font-size: 10px;
                    height: 32px;
                  ...
                  "
                  @click="userKick(member.studyMember.userno)"
                  >강퇴</b-button
                >
              </div>
            </div>
            <a class="kick" type="button" @click="userKick(userNumber)"
              >스터디 탈퇴</a
            >
          </b-sidebar>
          ...
          <b-sidebar id="sidebar-right" title="채팅 게시판" right shadow>
            <div class="px-3 py-2" id="chatting"></div>
            <div>
          ...
          <router-link :to="{ name: 'Main' }">
            <h4 class="exit" style="color: white">
              <font-awesome-icon icon="door-closed" />exit
            </h4></router-link
          >
        </div>
        <div id="wrapper">
          <div id="join" class="animate join">
            <form v-on:submit.prevent accept-charset="UTF-8">
              <p class="submit">
                <button @click="register" value="Join!" style="width: 100px">
                  화상회의 시작!
                </button>
              </p>
            </form>
        ...
        <video id="videoaa" autoplay></video>  class="d-flex align-items-start" -->
            ...
            <div id="buttons" style="display: none">
              ...
              <button id="muteoff" @click="muteoff" style="display: none">
                음성 켜기
              </button>
              <button id="videooff" @click="videooff">비디오 중지</button>
              <button id="videoon" @click="videoon" style="display: none">
                비디오 켜기
              </button>
              <button id="screenshare" @click="presenter">화면 공유</button>
              <button id="stop" @click="stop" style="display: none">
                화면 공유 종료
              </button>
              <button id="button-leave" @click="leaveRoom" value="Leave room">
                나가기
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    sidebarHomeworkOn() {
      ...
      this.sidebarMember = false;
      this.sidebarHomework = !this.sidebarHomework;
    },
    sidebarArticleOn() {
      this.sidebarHomework = false;
      this.sidebarDetail = false;
      this.sidebarMember = false;
    ...
    sidebarDetailOn() {
      ...
      this.sidebarArticle = false;
      this.sidebarMember = false;
      this.sidebarDetail = !this.sidebarDetail;
    },
    sidebarMemberOn() {
      this.sidebarHomework = false;
      this.sidebarArticle = false;
      this.sidebarDetail = false;
      this.sidebarMember = !this.sidebarMember;
    },
    ...
    getStudyInformation: function () {
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}study/detail/${this.$route.params.study_no}`,
    ...
    getArticleList: function () {
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}study_board/boardlist/${this.$route.params.study_no}`,
        headers: this.setToken(),
      })
        .then((res) => {
    ...
    getHomeworkList: function () {
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}homework/homeworklist/${this.$route.params.study_no}`,
        headers: this.setToken(),
      })
        .then((res) => {
    ...
    getStudyMemberList: function () {
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}study/memberList/${this.$route.params.study_no}`,
        headers: this.setToken(),
      })
        .then((res) => {
    ...
    getCalendar: function () {
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}study/calendar/${this.$route.params.study_no}`,
      })
        .then((res) => {
    ...
    userKick: function (userno) {
      ...
      axios({
        method: "post",
        url: `${process.env.VUE_APP_SERVER_URL}admin/kick`,
        data: this.userInfo,
      })
        .then((res) => {
    ...
    getLeader: function () {
      for (
        let index = 0;
        index < this.studyMemberList.studyMemberList.length;
        index++
      ) {
        if (
          this.studyMemberList.studyMemberList[index].studyMember.position == 0
        ) {
          this.leader =
            this.studyMemberList.studyMemberList[index].studyMember.userno;
          return this.leader;
        }
      }
    },
...
function Participant(name) {
  ...
  var createMainDivStyle =
    "border-radius: 4px; margin-left: 5px; margin-right: 5px;	width: 150px;	text-align: center;overflow: hide;	float: left;padding: 5px;border-radius: 10px;";
...
function leaveRoom() {
  ...
  axios({
    method: "post",
    url: `${process.env.VUE_APP_SERVER_URL}study/time/`,
    data: data,
  })
    .then((res) => {
      console.log(res);
