<template>
  <div>
    <h4 class="mb-3" style="font-weight: bold; font-size: 1.3rem">
      오늘 진행되는 스터디
    </h4>
...
export default {
  ...
  methods: {
    dayToNum(day) {
      ...
      var res =
        Number(beforenum[0]) * 365 +
        months[Number(beforenum[1]) - 1] +
        Number(beforenum[2]);
      return res;
    },
  },
    ...
    getStudyList() {
      ...
      for (var i = 0; i < this.studyList.length; i++) {
        for (var j = 0; j < this.studyList[i].scheduleList.length; j++) {
          if (
            this.dayToNum(this.studyList[i].scheduleList[j].nextDate) ==
            todaynum
          ) {
