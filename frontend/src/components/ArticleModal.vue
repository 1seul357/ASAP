<template>
  <div class="container">
    <font-awesome-icon class="fa-2x mt-1 mr-1 float-left" icon="edit" /><a
      type="button"
      id="show-btn"
      class="float-left mt-2"
      style="font-size: 20px; color: #394e79"
      @click="showModal"
      ><strong>글 작성하기</strong></a
    >
    <b-modal ref="my-modal" ok-only title="Create Ariticle" hide-footer>
      <form>
        ...
        <b-form-group label="Content" label-for="content">
          ...
          <b-button
            type="submit"
            class="mt-5"
            style="float: right; background-color: rgb(130, 163, 209)"
            @click="createArticle()"
            >확인</b-button
          >
        </b-form-group>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      (this.article.title = null), (this.article.content = null);
    ...
    createArticle: function () {
      ...
      if (
        ...
        axios({
          method: "post",
          url: `${process.env.VUE_APP_SERVER_URL}study_board/create`,
