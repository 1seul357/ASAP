    ...
    <div class="container">
      ...
      <div class="form col-9">
        <div class="d-flex justify-content-center">
          <div>
            <font-awesome-icon class="fa-3x mt-2 mr-3" icon="bell" />
          </div>
          <div>
            <p class="mb-0">
              모집할 스터디를 선택하고, 필요한 정보를 입력해주세요.
            </p>
            <p class="float-left">
              스터디 git 주소와 오픈 카카오톡은 선택사항입니다.
            </p>
          </div>
        </div>
        <b-form>
          <b-form-group>
            <label class="float-left" for="study">스터디</label>
            <study-dropdown v-on:studyEvent="updateStudy"></study-dropdown>
          </b-form-group>
          <b-form-group
            ...
            class="mt-4"
          >
            <label class="float-left" for="title">스터디 이름</label>
            <b-form-input id="input-title" v-model="title" type="text" required
              >{{ title }}
            </b-form-input>
          </b-form-group>
          ...
          <b-form-group id="input-git-group" class="mt-4" label-for="input-git">
            <label class="float-left" for="input-description"
              >스터디 링크</label
            >
            <b-form-input
          ...
          <div class="mt-3">
            <b-button @click="createStudyRoom" class="float-right"
              >작성하기</b-button
            >
          </div>
        </b-form>
...
export default {
  ...
  methods: {
    updateStudy(studyinfo) {
      ...
      axios({
        method: "get",
        url: `${process.env.VUE_APP_SERVER_URL}study/list/simple-detail/${studyinfo.studyno}`,
        data: this.study,
      }).then((res) => {
        console.log(res);
    ...
    createStudyRoom: function () {
      ...
      for (var j = 0; j < StudyRoomItem.boarddescription.length; j++) {
        if (StudyRoomItem.boarddescription[j] != " ") {
          flagContent = true;
          break;
        }
      }
      ...
      if (
        StudyRoomItem.boardname == null ||
        StudyRoomItem.boarddescription == null ||
        StudyRoomItem.boardname == "" ||
        StudyRoomItem.boarddescription == "" ||
        !flagTitle ||
        !flagContent
      ) {
        ...
        axios({
          method: "post",
          url: `${process.env.VUE_APP_SERVER_URL}board/create/`,
          data: StudyRoomItem,
        })
          .then((res) => {
            console.log(res);
